...
import clip as clip1
from models.clip import build_model
from models.unpair import TransformerEncoder, TransformerDecoder, Transformer
    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader)) as pbar:
        with torch.no_grad():
            for it, batch in enumerate(dataloader):
                ...
                feat = clip.encode_text(clip1.tokenize(text_field.decode(captions)).to(device))
                out = model(feat, captions)
                optim.zero_grad()
...
def evaluate_metrics(model, dataloader, text_field):
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        for it, batch in enumerate(dataloader):
            image_id, samples, caps_gt = batch['image_id'], batch['samples'].to(device), batch['caps_gt']
            with torch.no_grad():
...
if __name__ == '__main__':
    ...
    args = OmegaConf.load('configs/unpair.yaml')
    print(args)
    ...
    # if test:
    #     sys.exit()
    model0 = torch.jit.load(open("/home/liuyuanqiu/code/ImgCap/ckpts/clip/ViT-B-32.pt", 'rb'), map_location=device).eval()
    clip = build_model(model0.state_dict()).to(device)
    ...
    model = Transformer(text_field.vocab.stoi['<bos>'], clip, encoder, decoder).to(device)
