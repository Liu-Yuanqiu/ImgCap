...
def infer(model, dataloader, text_field):
    ...
    with tqdm(desc='evaluation', unit='it', total=len(dataloader)) as pbar:
        for it, batch in enumerate(dataloader):
            samples, image_ids = batch['samples'], batch['image_id']
            if origin_fea == "swin_dert_grid":
                feat, mask = samples["grid"].to(device), samples["mask"].to(device)
            elif origin_fea == "swin_dert_region":
                feat, mask = samples["region_sd"].to(device), samples["region_sd_mask"].to(device)
            elif origin_fea == "up_down_36":
                feat, mask = samples["region_ud"].to(device), None
            ...
            for i, (id, gen_i) in enumerate(zip(image_ids, caps_gen)):
                d = {}
                d['image_id'] = int(id)
                d['caption'] = gen_i
                data.append(d)
...
if __name__ == '__main__':
    ...
    with open("./captions_test2014_results_"+origin_cap+".json", "w") as f:
        json.dump(data4w, f)
    ...
    with open("./captions_val2014_results_"+origin_cap+".json", "w") as f:
        json.dump(data3w, f)