...
class Transformer(nn.Module):
    def __init__(self, feat_dim, vocab_size, padding_idx, topk, \
        ...
        self.de = nn.ModuleList([DiffusionDecoderLayer(d_model, d_k, d_v, h, d_ff, dropout) for _ in range(N_de)])
        self.de_pos_emb = nn.Embedding.from_pretrained(sinusoid_encoding_table(200, d_model, 1), freeze=True)
    ...
    def forward(self, feat, feat_mask, labels, tokens_kd):
        ...
        for layer in self.ew:
            outw = layer(outw, outw, ew_batch_times_emb, feat, feat_mask)
        ...
        de_noise = torch.randn_like(tokens_kd_emb + self.de_pos_emb(pos_indx))
        de_batch_times = torch.randint(0, self.num_timesteps, (bs,), device=device)
        de_x = self.q_sample(x_start=tokens_kd_emb, t=de_batch_times, noise=de_noise)
        out = de_x.to(torch.float32)
        de_batch_times_emb = self.ew_time_emb(de_batch_times)
        for layer in self.de:
            out = layer(out, outw, de_batch_times_emb, feat, feat_mask)
        if self.objective == 'pred_noise':
            de_target = ew_noise
        elif self.objective == 'pred_x0':
            de_target = tokens_kd_emb
        elif self.objective == 'pred_v':
            de_v = self.predict_v(tokens_kd_emb, de_batch_times, de_noise)
            de_target = de_v
        ...
        losses.update({"de_mse": F.mse_loss(out, de_target)})
    ...
    def infer(self, feat, feat_mask):
        ...
        de_x = torch.randn((bs, self.topk, self.dim), device=device)
        de_x_start = None
        for t in reversed(range(0, self.num_timesteps)):
            de_batched_times = torch.full((bs,), t, device = device, dtype = torch.long)
                    
            de_batched_times_emb = self.ew_time_emb(de_batched_times)
            for l in self.de:
                de_x = l(de_x, outw, de_batched_times_emb, feat, feat_mask)

            if self.objective == 'pred_noise':
                de_pred_noise = de_x
                de_x_start = self.predict_start_from_noise(de_x, de_batched_times, de_pred_noise)
            elif self.objective == 'pred_x0':
                de_x_start = de_x
                de_pred_noise = self.predict_noise_from_start(de_x, de_batched_times, de_x_start)
            elif self.objective == 'pred_v':
                de_v = de_x
                de_x_start = self.predict_start_from_v(de_x, de_batched_times, de_v)
                de_pred_noise = self.predict_noise_from_start(de_x, de_batched_times, de_x_start)
            
            de_x_start.clamp_(-1., 1.)
            
            de_model_mean = (
                extract(self.posterior_mean_coef1, de_batched_times, de_x.shape) * de_x_start +
                extract(self.posterior_mean_coef2, de_batched_times, de_x.shape) * de_x
            )
            # posterior_variance = extract(self.posterior_variance, batched_times, x.shape)
            de_model_log_variance = extract(self.posterior_log_variance_clipped, de_batched_times, de_x.shape)
            de_noise = torch.randn_like(de_x) if t > 0 else 0. # no noise if t == 0
            de_x = de_model_mean + (0.5 * de_model_log_variance).exp() * de_noise
        out = self.unnormalize(de_x)
        logit = self.de_fc(out)
...
class DiffusionDecoderLayer(nn.Module):
    ...
    def forward(self, x, x1, c, vis, vis_mask):
        ...
        input_x1 = modulate(self.norm1(x1), shift_msa, scale_msa)
        x = x + gate_msa.unsqueeze(1) * self.self_att(input, input_x1, input_x1)
