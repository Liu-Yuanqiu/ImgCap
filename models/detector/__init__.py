...
from .det_model import build_det_module_with_config


class Swin_Det_Detector(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.use_reg_feat:
            ...
            hs, init_reference, boxes = self.det_module(srcs, masks)
            outputs['reg_feat'] = hs[-1]
            outputs['reg_mask'] = hs[-1].data.new_full((hs[-1].shape[0], 1, 1, hs[-1].shape[1]), 0).bool()
            
            # 0402tianjia
            det_out = self.det_module.detection_head(hs, init_reference, boxes)
            pred_logits = det_out['pred_logits']
            label_v, label_idx = torch.max(pred_logits, -1)
            _, label_idx_topk = torch.topk(label_v, 10)
            outputs['object_topk'] = torch.gather(label_idx, -1, label_idx_topk)
        return outputs


def build_detector(config):
    pos_dim = getattr(config.model.swin, 'pos_dim', None)
    backbone, _ = swin_base_win7_384(
        frozen_stages=config.model.swin.frozen_stages,
        pos_dim=pos_dim,
    )
    det_cfg = config.model.det
    det_module = build_det_module_with_config(det_cfg) if config.model.use_reg_feat else None
    detector = Swin_Det_Detector(
        backbone,
    ...
    if os.path.exists(config.model.det.checkpoint):
        checkpoint = torch.load(config.model.det.checkpoint, map_location='cpu')
